...
def __build_document_from_manifests(
    ...
    with open(document_path, "w") as out_writer:
        for manifest in manifests:
            with open(DataStoreObject(manifest).get()) as in_reader:
        ...
        logging.info(f"Finished extracting all manifests ! Number of sentences : {num_lines}")
    return document_path
...
def __process_data(
    ...
    if tokenizer_type == "spe":
        ...
        if spe_max_sentencepiece_length > 0:
            tokenizer_dir = os.path.join(dst_folder, "tokenizer_{}_{}_v{}_max_{}").format(tokenizer_type, spe_type, vocab_size, spe_max_sentencepiece_length)
        else:
...
def process_asr_text_tokenizer(
    manifests=None,
    data_file=None,
    data_root=None,
    vocab_size=1024,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Type of the SentencePiece model. Can be `bpe`, `unigram`, `char` or `word`." "Used only if --tokenizer == `spe`",
    )
    parser.add_argument(
        "--spe_character_coverage",
        type=float,
        default=1.0,
        help="Character coverage percentage for SentencePiece tokenization. For languages " "with large vocabulary, should be close to 0.9995, otherwise kept as 1.0",
    )
    ...
    parser.add_argument("--spe_user_defined_symbols", default=None, type=str, nargs="+", help="User defined symbols for SentencePiece")
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Limit the maximum number of tokens in each SentencePiece subword. " "Must be a positive integer > 0. By default places no limit on subword length.",
    )
    parser.add_argument(
    ...
    process_asr_text_tokenizer(
        ...
        spe_bos=args.spe_bos,
        spe_eos=args.spe_eos,
        spe_pad=args.spe_pad,
