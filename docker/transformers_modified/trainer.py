...
class Trainer:
    ...
    def __init__(
        ...
        if model is None:
            ...
            if model_init is not None:
                warnings.warn(
                    "`Trainer` requires either a `model` or `model_init` argument, but not both. `model_init` will" " overwrite your model when calling the `train` method. This will become a fatal error in the next" " release.",
                    FutureWarning,
                )
            self.model_init = model_init
        ...
        if len(args.sharded_ddp) > 0:
            if args.deepspeed:
                raise ValueError("Using --sharded_ddp xxx together with --deepspeed is not possible, deactivate one of those flags.")
            if len(args.fsdp) > 0:
                raise ValueError("Using --sharded_ddp xxx together with --fsdp is not possible, deactivate one of those flags.")
            ...
            if args.local_rank == -1:
                ...
                raise ImportError("Sharded DDP in a mode other than simple training requires fairscale version >= 0.3, found " f"{fairscale.__version__}. Upgrade your fairscale library: `pip install --upgrade fairscale`.")
        ...
        if self.is_model_parallel or args.deepspeed or ((args.fp16_full_eval or args.bf16_full_eval) and not args.do_train) or (self.sharded_ddp in [ShardedDDPOption.ZERO_DP_2, ShardedDDPOption.ZERO_DP_3]) or (self.fsdp is not None):
        ...
        if ((self.sharded_ddp is not None) or args.deepspeed or (self.fsdp is not None)) and (self.optimizer is not None or self.lr_scheduler is not None):
            raise RuntimeError("Passing `optimizers` is not allowed if Fairscale, Deepspeed or PyTorch FSDP is enabled." "You should subclass `Trainer` and override the `create_optimizer_and_scheduler` method.")
        ...
        if is_sagemaker_mp_enabled():
            ...
            if IS_SAGEMAKER_MP_POST_1_10:
                ...
                if args.fp16 != smp.state.cfg.fp16:
                    logger.warning(f"FP16 provided in SM_HP_MP_PARAMETERS is {smp.state.cfg.fp16}," f"but FP16 provided in trainer argument is {args.fp16}," f"setting to {smp.state.cfg.fp16}")
        ...
        if is_sagemaker_mp_enabled() and self.use_cuda_amp and args.max_grad_norm is not None and args.max_grad_norm > 0:
            raise ValueError("SageMaker Model Parallelism in mixed precision mode does not support gradient clipping yet. Pass " "along 'max_grad_norm': 0 in your hyperparameters.")
        ...
        # Distributed training using PyTorch FSDP
            ...
            if FSDPOption.AUTO_WRAP in self.args.fsdp:
                if self.args.fsdp_min_num_params > 0:
                    auto_wrap_policy = functools.partial(size_based_auto_wrap_policy, min_num_params=self.args.fsdp_min_num_params)
    ...
    def _inner_training_loop(self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None):
        ...
        if DebugOption.UNDERFLOW_OVERFLOW in self.args.debug:
            if self.args.n_gpu > 1:
                ...
                raise ValueError("Currently --debug underflow_overflow is not supported under DP. Please use DDP" " (torch.distributed.launch).")
            else:
                debug_overflow = DebugUnderflowOverflow(self.model)  # noqa

        delay_optimizer_creation = self.sharded_ddp is not None and self.sharded_ddp != ShardedDDPOption.SIMPLE or is_sagemaker_mp_enabled() or self.fsdp is not None
        if args.deepspeed:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            step = -1
            SKIPPING = steps_trained_in_current_epoch > 0
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        if not os.path.isfile(os.path.join(resume_from_checkpoint, WEIGHTS_NAME)) and not os.path.isfile(os.path.join(resume_from_checkpoint, WEIGHTS_INDEX_NAME)):
        ...
        if os.path.isfile(os.path.join(resume_from_checkpoint, CONFIG_NAME)):
            ...
            if checkpoint_version is not None and checkpoint_version != __version__:
                logger.warning(
                    f"You are resuming training from a checkpoint trained with {checkpoint_version} of " f"Transformers but your current version is {__version__}. This is not recommended and could " "yield to errors or unwanted behaviors."
                )
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        checkpoint_file_exists = glob.glob(os.path.join(checkpoint, OPTIMIZER_NAME) + "_*") if is_sagemaker_mp_enabled() else os.path.isfile(os.path.join(checkpoint, OPTIMIZER_NAME))
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        if backend is None:
            ...
            if backend is None:
                raise RuntimeError("At least one of optuna or ray should be installed. " "To install optuna run `pip install optuna`. " "To install ray run `pip install ray[tune]`. " "To install sigopt run `pip install sigopt`.")
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if len(inputs) == 0:
            raise ValueError("The batch received was empty, your model won't be able to train on it. Double-check that your " f"training dataset contains keys expected by the model: {','.join(self._signature_columns)}.")
    ...
    def autocast_smart_context_manager(self, cache_enabled: Optional[bool] = True):
        ...
        if self.use_cuda_amp or self.use_cpu_amp:
            if is_torch_greater_or_equal_than_1_10:
                ctx_manager = torch.cpu.amp.autocast(cache_enabled=cache_enabled, dtype=self.amp_dtype) if self.use_cpu_amp else torch.cuda.amp.autocast(cache_enabled=cache_enabled, dtype=self.amp_dtype)
            else:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            ...
            if isinstance(outputs, dict) and "loss" not in outputs:
                raise ValueError("The model did not return a loss from the inputs, only the following keys: " f"{','.join(outputs.keys())}. For reference, the inputs it received are {','.join(inputs.keys())}.")
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        elif ShardedDDPOption.ZERO_DP_2 in self.args.sharded_ddp or ShardedDDPOption.ZERO_DP_3 in self.args.sharded_ddp or self.fsdp is not None:
    ...
    def _push_from_checkpoint(self, checkpoint_folder):
        ...
        try:
            ...
            _, self.push_in_progress = self.repo.push_to_hub(commit_message=commit_message, blocking=False, auto_lfs_prune=True)
        finally:
            if self.args.hub_strategy == HubStrategy.CHECKPOINT:
    ...
    def push_to_hub(self, commit_message: Optional[str] = "End of training", blocking: bool = True, **kwargs) -> str:
        ...
        git_head_commit_url = self.repo.push_to_hub(commit_message=commit_message, blocking=blocking, auto_lfs_prune=True)
        # push separately the model card to be independant from the rest of the model
        if self.args.should_save:
            self.create_model_card(model_name=model_name, **kwargs)
            try:
                self.repo.push_to_hub(commit_message="update model card README.md", blocking=blocking, auto_lfs_prune=True)
