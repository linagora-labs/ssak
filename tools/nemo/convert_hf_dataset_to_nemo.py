...
def prepare_output_dirs(cfg: HFDatasetConversionConfig):
    ...
    output_dir = os.path.join(output_dir, os.path.basename(cfg.path))
...
def convert_offline_dataset_to_nemo(
    ...
    num_proc = min(os.cpu_count() // 2, 4) if os.cpu_count() > 2 else 1
    ...
    with open(manifest_filepath, "w") as manifest_f:
        for idx, sample in enumerate(tqdm.tqdm(ds_iter, desc=f"Processing {cfg.path} (split : {cfg.split}):", total=len(dataset), unit=" samples")):
...
def convert_streaming_dataset_to_nemo(dataset: IterableDataset, cfg: HFDatasetConversionConfig, basedir: str, manifest_filepath: str):
    """
    ...
    with open(manifest_filepath, "w") as manifest_f:
        for idx, sample in enumerate(tqdm.tqdm(ds_iter, desc=f"Processing {cfg.path} (split: {cfg.split}):", unit=" samples")):
