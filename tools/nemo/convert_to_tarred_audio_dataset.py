...
class ASRTarredDatasetBuilder:
    ...
    def create_new_dataset(
        ...
        for i in range(config.num_shards):
            ...
            print(f"Shard {i} contains {end_idx-start_idx+1} files")
        ...
        with Parallel(n_jobs=num_workers, verbose=config.num_shards) as parallel:
            ...
            new_entries_list = parallel(delayed(self._create_shard)(entries[start_idx:end_idx], target_dir, i, manifest_folder) for i, (start_idx, end_idx) in enumerate(zip(start_indices, end_indices)))
        ...
        if buckets_num <= 1:
            ...
            bucketing_kwargs = self.estimate_dynamic_bucketing_duration_bins(new_manifest_path, num_buckets=dynamic_buckets_num)
    ...
    def create_concatenated_dataset(
        ...
        for new_manifest_idx in range(len(manifest_paths)):
            new_entries, total_duration, filtered_new_entries, filtered_duration = self._read_manifest(manifest_paths[new_manifest_idx], config)

            if len(filtered_new_entries) > 0:
                print(f"Filtered {len(filtered_new_entries)} files which amounts to {filtered_duration:0.2f}" f" seconds of audio from manifest {manifest_paths[new_manifest_idx]}.")
            print(f"After filtering, manifest has {len(entries)} files which amounts to {total_duration} seconds of audio.")
        ...
        print(f"Dropping {drop_count} samples from total new samples {total_new_entries} since they cannot " f"be added into a uniformly sized chunk.")
        ...
        with Parallel(n_jobs=num_workers, verbose=num_added_shards) as parallel:
            # Call parallel tarfile construction
            new_entries_list = parallel(delayed(self._create_shard)(entries[start_idx:end_idx], target_dir, shard_idx, manifest_folder) for i, (start_idx, end_idx, shard_idx) in enumerate(zip(start_indices, end_indices, shard_indices)))
        ...
        print(
            ...
            len(base_entries) + len(new_entries),
        )
    ...
    def _write_to_tar(
        ...
        if (codec := self.config.force_codec) is None or audio_filepath.endswith(f".{codec}"):
            ...
            if duration is None or offset <= 0.1:
                ...
                audio = audio[int(offset * sampling_rate) : int((offset + duration) * sampling_rate)]
                encoded_audio = BytesIO()
                soundfile.write(encoded_audio, audio, sampling_rate, format="wav")
                ti = tarfile.TarInfo(squashed_filename)
...
def create_tar_datasets(
    ...
    concat_manifest_paths: Optional[list[str]] = None,
    metadata_path: Optional[str] = None,
):
    if os.path.exists(os.path.join(target_dir, "metadata.yaml")):
        print(f"Skipping bucket {min_duration:.2f}->{max_duration:.2f} because metadata file already exists. Please remove it if you want to regenerate.")
        return
...
def convert_to_tarred_audio_dataset(
    ...
    buckets_num=1,
    dynamic_buckets_num=30,
    keep_files_together=False,
    force_codec=None,
    workers=1,
    ...
    if buckets_num > 1:
        ...
        for i in range(buckets_num):
            ...
            create_tar_datasets(
                min_duration=bucket_min_duration,
                max_duration=bucket_max_duration,
                ...
                no_shard_manifests=no_shard_manifests,
                manifest_path=manifest_path,
                concat_manifest_paths=concat_manifest_paths,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert an existing ASR dataset to tarballs compatible with TarredAudioToTextDataLayer.")
    parser.add_argument(
        "--manifest_path",
        default=None,
        type=str,
        required=False,
        help="Path to the existing dataset's manifest.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Intended for dynamic (on-the-fly) bucketing; this option will not bucket your dataset during tar conversion. " "Estimates optimal bucket duration bins for a given number of buckets.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Random seed for use if shuffling is enabled.",
    )
    parser.add_argument(
        "--write_metadata",
        action="store_true",
        help=("Flag to write a blank metadata with the current call config. " "Note that the metadata will not contain the number of shards, " "and it must be filled out by the user."),
    )
    parser.add_argument(
        "--no_shard_manifests",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("If specified, transcode the audio to the given format. " "Supports libnsndfile formats (example values: 'opus', 'flac')."),
    )
    parser.add_argument("--workers", type=int, default=1, help="Number of worker processes")
    args = parser.parse_args()
    print("Lauching conversion")
    convert_to_tarred_audio_dataset(
        ...
        dynamic_buckets_num=args.dynamic_buckets_num,
        keep_files_together=args.keep_files_together,
        force_codec=args.force_codec,
        workers=args.workers,
        write_metadata=args.write_metadata,
        no_shard_manifests=args.no_shard_manifests,
